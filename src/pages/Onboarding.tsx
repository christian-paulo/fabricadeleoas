    ...
    <div className="min-h-screen bg-background flex flex-col max-w-lg mx-auto">
      ...
      <div className="px-4 pb-8 flex gap-3">
        ...
        >
          ...
          {step < totalSteps - 1 && <ChevronRight size={18} className="ml-1" />}
        </Button>
      </div>
    </div>
  );
...
function getStepCount(data: QuizData): number {
  let count = 6; // base steps: name, goal, area, exp, days, duration
  if (data.goal !== "Melhorar Dores") count++; // pain question
  if (data.hasPain) count++; // pain location
  count++; // medication question
  if (data.usesMedication) count++; // medication feeling
  return count;
}
...
function buildSteps(data: QuizData, setData: (d: QuizData) => void) {
      ...
      <div className="space-y-3">
        {goals.map((g) => (
          <button
            ...
            className={`neu-card w-full p-4 text-left text-sm transition-all ${
              data.goal === g ? "ring-2 ring-primary text-primary" : "text-foreground"
            }`}
          >
            {g}
          </button>
            ...
            <button
              ...
              onClick={() =>
                setData({
                  ...data,
                  targetArea: selected
                    ? data.targetArea.filter((x) => x !== a)
                    : [...data.targetArea, a],
                })
              }
              className={`neu-card w-full p-4 text-left text-sm transition-all ${
                selected ? "ring-2 ring-primary text-primary" : "text-foreground"
              }`}
            >
              {a}
            </button>
    ...
    <div key="exp">
      ...
      <div className="space-y-3">
        {expOptions.map((e) => (
          <button
            key={e}
            onClick={() => setData({ ...data, trainingExperience: e })}
            className={`neu-card w-full p-4 text-left text-sm transition-all ${
              data.trainingExperience === e ? "ring-2 ring-primary text-primary" : "text-foreground"
            }`}
          >
            {e}
          </button>
    ...
    <div key="days">
      ...
      <div className="flex gap-3 justify-center">
        {[2, 3, 4, 5].map((d) => (
          <button
            key={d}
            onClick={() => setData({ ...data, workoutDays: d })}
            className={`w-14 h-14 rounded-xl font-heading text-lg transition-all ${
              data.workoutDays === d
    ...
    <div key="duration">
      ...
      <div className="flex gap-4 justify-center">
        {durations.map((d) => (
          <button
            key={d}
            onClick={() => setData({ ...data, workoutDuration: d })}
            className={`flex-1 max-w-[140px] h-20 rounded-xl font-heading text-lg transition-all ${
  ...
  // 6 - Pain question (if goal != "Melhorar Dores")
      ...
      <div key="pain">
        ...
        <div className="flex gap-4 justify-center">
          {[
            ...
            <button
              key={String(opt.val)}
              onClick={() => setData({ ...data, hasPain: opt.val, painLocation: opt.val ? data.painLocation : [] })}
              className={`flex-1 max-w-[140px] h-16 rounded-xl font-heading transition-all ${
                data.hasPain === opt.val
                  ? "gold-gradient text-primary-foreground gold-glow"
                  : "neu-card text-foreground"
              }`}
            >
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    setData({
                      ...data,
                      painLocation: selected
                        ? data.painLocation.filter((x) => x !== p)
                        : [...data.painLocation, p],
                    })
                  }
                  className={`neu-card p-3 text-sm text-left transition-all ${
                    selected ? "ring-2 ring-accent text-accent" : "text-foreground"
                  }`}
                >
                  {p}
                </button>
    ...
    <div key="med">
      ...
      <div className="flex gap-4 justify-center">
        {[
          ...
          <button
            key={String(opt.val)}
            onClick={() => setData({ ...data, usesMedication: opt.val })}
            className={`flex-1 max-w-[140px] h-16 rounded-xl font-heading transition-all ${
              data.usesMedication === opt.val
                ? "gold-gradient text-primary-foreground gold-glow"
                : "neu-card text-foreground"
            }`}
          >
  ...
  if (data.usesMedication) {
    steps.push(
      <div key="medFeel">
        ...
        <textarea
          value={data.medicationFeeling}
          onChange={(e) => setData({ ...data, medicationFeeling: e.target.value })}
